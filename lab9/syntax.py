...
def detect_syntax(text: str, lang: str):
    client = boto3.client("comprehend")
    resp = client.detect_syntax(Text=text, LanguageCode=lang)
    for tok in resp["SyntaxTokens"]:
        print(f"{tok['Text']:<20}  {tok['PartOfSpeech']['Tag']}  ({tok['PartOfSpeech']['Score']:.2f})")
